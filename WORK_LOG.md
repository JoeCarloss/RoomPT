...
**막판 카운트 정확도 2건 추가 수정**: (6) 무릎각 노이즈로 일어설 때 중간값(105~150°)을 지날 때마다 up-스트릭이 리셋돼 카운트 실패 → 스트릭을 노이즈 내성 있게(중간값 유지 + 반대 구간 감쇠) 변경. (7) `shoulderWidth>0.015` 카운트 가드가 측면(어깨 앞뒤로 겹쳐 폭~0.01)을 막고 정면 회전 시에만 통과시켜 "회전해야 카운트" 버그 유발 → 제거(bodyPlausible 게이트가 이미 쓰레기 프레임 차단하므로 불필요).

**최종 상태(2026-07-19, 사용자 확인 완료)**: 전신 인식 후 진짜 스쿼트 정확히 카운트 O(숫자 일치 확인), 팔만/부분/회전 카운트 X. `DEBUG_LOG=false`로 끔(코드는 남겨둬 자세-교정 튜닝 때 재사용). **자세 교정은 현재 미작동** — 무릎/엉덩이 각도(회전 불변) 외의 세로(y)·좌우·폭 기반 검사(고개 처짐/좌우 기울기/골반/무릎 모임/스탠스)는 프레임 가로 회전 + 원거리 때문에 깨지거나 억제됨. **다음 작업**: 랜드마크를 촬영 방향에 맞게 회전 보정한 뒤 자세 검사 되살리기(또는 전부 각도 기반으로 재작성). 카메라 orientation 근본 교정도 함께 검토.
